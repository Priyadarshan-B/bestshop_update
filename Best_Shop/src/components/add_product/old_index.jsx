...
function AddStocks({ text }) {
  ...
  const addFieldDetails = async () => {
    try {
      ...
      const fieldId = document.getElementById("field_name").value.split(",")[0];
      ...
      const response = await fetch(`${apiHost}/field-details`, {
        method: "POST",
        body: formData,
      });
      ...
      // Assuming fetchOptions and fetchFields are asynchronous
      await fetchOptions(data.category_id, fieldId); // Pass the correct values for id and field_id
      await fetchFields();
  ...
  const handleSearch = () => {
    const success = window.find(searchQuery);
    if (!success) {
