...
exports.placeOrder = async (req, res) => {
    ...
    const ordered_date = new Date().toISOString().slice(0, 19).replace('T', ' ');
    ...
    try {
        const query = `UPDATE purchase_requests SET status = '2', ordered_date = ?, received_date = NULL WHERE id = ?;`;
        const values = [ordered_date, id];

        const result = await post_query_database(query, values);

        res.json({
            message: "Purchase request status updated to '2' successfully",
            result,
        });
    } catch (err) {
        console.error(`Error: ${err.message}`);
...
exports.receiveOrder = async (req, res) => {
    ...
    const received_date = new Date().toISOString().slice(0, 19).replace('T', ' ');
    ...
    try {
        const query = `UPDATE purchase_requests SET status = '3', received_date = ? WHERE id = ?;`;
        const values = [received_date, id];

        const result = await post_query_database(query, values);

        res.json({
            message: "Order received. Status updated to '3' successfully",
            result,
        });
    } catch (err) {
        console.error(`Error: ${err.message}`);
...
exports.getPendingOrders = async (req, res) => {
    ...
    pr.sub_category AS sub_category,
    pr.requested_date AS requested_date,
    pr.ordered_date AS ordered_date,
    pr.supplier AS supplier
FROM 
    purchase_requests pr
LEFT JOIN 
...
`;
        ...
        if (result.length > 0) {
            ...
            res.status(404).json({ message: "No orders received" });
        }
    } catch (err) {
        console.error(`Error: ${err.message}`);
        res.status(500).send("Internal server error");
